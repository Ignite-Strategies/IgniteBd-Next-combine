...
| **emailSequenceOrder** | SENT \| CONTACT_RESPONDED \| OWNER_RESPONSE — who sent this row. |
...
1. Create **first** row (e.g. SENT). `responseFromEmail` = null.
2. Create **second** row (e.g. CONTACT_RESPONDED). Then **update first** row: `responseFromEmail` = second.id.
3. Create **third** row (e.g. OWNER_RESPONSE). Then **update second** row: `responseFromEmail` = third.id.
