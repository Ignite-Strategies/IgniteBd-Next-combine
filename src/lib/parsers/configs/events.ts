...
export const parserConfig: ParserConfig = {
  ...
  schema: eventSelectionSchema,

  systemPrompt: `You are a structured data extraction engine.

Your job is to analyze unstructured text about event selection and extract factual information into a strict JSON object.
...
Return ONLY JSON. No markdown, no explanations, no commentary.`,
  ...
  buildUserPrompt: (raw: string, context: string | null) => `Extract the event selection fields from the following raw text.
