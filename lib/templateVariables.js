...
export function hydrateTemplate(template, contactData = {}, metadata = {}) {
  // Ensure contactData and metadata are objects (handle null/undefined)
  const safeContactData = contactData && typeof contactData === 'object' ? contactData : {};
  const safeMetadata = metadata && typeof metadata === 'object' ? metadata : {};
  
  // Ensure template is a string
  if (typeof template !== 'string') {
    console.warn('hydrateTemplate: template must be a string, got:', typeof template);
    return template || '';
  }
  ...
  const data = {
    // Contact fields - safely access properties
    ...
    // Calculated fields - safely access properties
