...
/**
 * Extract variable names from template content (returns simple string array)
...
/**
 * Parse variable tags from template content (returns detailed objects)
 * @param {string} content - Template content with {{variableName}} tags
 * @returns {Array<{name: string, type: string, description: string}>} Array of variable objects
 */
 ...
 * 
 * @deprecated Use variableMapperService.hydrateTemplateFromDatabase() instead.
 * This function requires contactData to be pre-fetched and passed in.
 * The variable mapper service queries the database automatically based on contactId/email.
 * 
 * @param {string} template - Template content with {{variableName}} tags
 * @param {Object} contactData - Contact data object (must be pre-fetched)
...
export function generateTemplateWithVariables(templateBase) {
  // Ensure templateBase is an object
  const safeTemplateBase = templateBase || {};
  const { relationship, typeOfPerson, whyReachingOut, whatWantFromThem } = safeTemplateBase;
  
  // Safely get string values (handle undefined/null)
  const safeWhyReachingOut = whyReachingOut || '';
  const safeWhatWantFromThem = whatWantFromThem || '';
  ...
  // Context based on whyReachingOut - use string concatenation instead of template literal to avoid evaluation
  if (safeWhyReachingOut) {
    template += safeWhyReachingOut + ' ';
  }
  ...
  // What you want from them - use string concatenation instead of template literal
  if (safeWhatWantFromThem) {
    template += ' — ' + safeWhatWantFromThem;
  } else {
