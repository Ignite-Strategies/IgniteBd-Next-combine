...
export function hydrateTemplate(template, contactData = {}, metadata = {}) {
  ...
  // Build the data object with contact info + metadata
  const data = {
    // Contact fields
    firstName: contactData.firstName || contactData.goesBy || 'there',
    lastName: contactData.lastName || '',
    fullName: contactData.fullName || `${contactData.firstName || ''} ${contactData.lastName || ''}`.trim() || 'there',
    companyName: contactData.companyName || 'your company',
    title: contactData.title || 'your role',
    email: contactData.email || '',
    
    // Calculated fields
    timeSinceConnected: calculateTimeSince(contactData.lastContactDate || contactData.updatedAt),
    
    // Boolean fields (convert to yes/no)
    knowledgeOfBusiness: metadata.knowledgeOfBusiness ? 'yes' : 'no',
    
    // Metadata fields
    timeHorizon: metadata.timeHorizon || 'soon',
    desiredOutcome: metadata.desiredOutcome || '',
    myBusinessName: metadata.myBusinessName || 'my business',
    myRole: metadata.myRole || 'consultant',
