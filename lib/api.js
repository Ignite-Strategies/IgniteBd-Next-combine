...
if (typeof window !== 'undefined') {
  ...
  async (config) => {
    // CRITICAL: OAuth login endpoint must NOT use Axios
    // /api/microsoft/login is a redirect endpoint, not an API endpoint
    // It redirects to Microsoft OAuth, which AJAX requests can't follow due to CORS
    // Skip interceptor for OAuth login - it should use window.location.href instead
    if (config.url?.includes('/api/microsoft/login')) {
      console.warn('⚠️ Axios: Skipping interceptor for OAuth login endpoint. Use window.location.href instead.');
      return config;
    }
