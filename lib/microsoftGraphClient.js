...
export async function getValidAccessToken(ownerId) {
  try {
    ...
    if (!microsoftAccount || !microsoftAccount.accessToken) {
...
export async function refreshAccessToken(ownerId) {
  try {
    ...
    const tenantId = microsoftAccount.microsoftTenantId || 'common';
    ...
    // Update tokens in MicrosoftAccount model
    await prisma.microsoftAccount.update({
      where: { ownerId },
      data: {
        accessToken: tokenResponse.accessToken,
        refreshToken: tokenResponse.refreshToken || microsoftAccount.refreshToken,
        expiresAt: expiresAt,
...
export async function isMicrosoftConnected(ownerId) {
  try {
    // Check if MicrosoftAccount exists (new model)
    const microsoftAccount = await prisma.microsoftAccount.findUnique({
      where: { ownerId },
      select: {
        id: true,
