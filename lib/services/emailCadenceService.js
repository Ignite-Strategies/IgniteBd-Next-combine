...
export async function calculateNextSendDate(contactId, config = {}) {
  ...
  // Stored date wins (don't move the date). User-set date (remind-me / nextContactedAt) defaults to UNRESPONSIVE; else keep existing purpose.
  ...
  if (storedDate) {
    ...
    return {
      ...
      nextContactNote: contact.nextContactNote ?? null,
      nextEngagementPurpose: userSetDate ? 'UNRESPONSIVE' : null, // null = keep existing purpose when we only had nextEngagementDate
    };
  }
  ...
  if (!respondedAfterLastSend) {
    // 1. Don't respond → auto 7 days (UTC). Noon UTC so EST sees correct calendar day (not "yesterday").
    const nextSendDate = new Date(lastSendDate);
    nextSendDate.setUTCDate(nextSendDate.getUTCDate() + AUTO_CADENCE_DAYS);
    nextSendDate.setUTCHours(12, 0, 0, 0);
    return {
      nextSendDate,
      lastSendDate,
...
export async function computeAndPersistNextEngagement(contactId) {
  try {
    ...
    // When cadence returns null purpose (stored nextEngagementDate only), keep existing purpose; otherwise set it.
